...
def phase4_data():
    ...
    gene_sets = {
        name: grp["index"].values - 1
        for name, grp in gs_long.groupby("set", sort=False)
    }
...
def test_mroast_active_proportions_match(phase4_data, set_statistic):
    ...
    np.testing.assert_array_equal(
        py_sorted["n_genes"].values, r_sorted["NGenes"].values
    )
    np.testing.assert_allclose(
    ...
    np.testing.assert_array_equal(
        py_sorted["direction"].values, r_sorted["Direction"].values
    )
...
def test_fry_single_set_matches_r(phase4_data):
    ...
    np.testing.assert_allclose(
        py["p_value"].values, r["PValue"].values, rtol=1e-6, atol=1e-12
    )
    np.testing.assert_allclose(
...
def test_inter_gene_correlation_returns_vif_and_corr(phase4_data):
    ...
    out = inter_gene_correlation(
        phase4_data["y"][:30, :], phase4_data["design"]
    )
...
def test_romer_pvalues_log_scale_match(phase4_data, set_statistic):
    ...
    for col_py, col_r in [("up", "Up"), ("down", "Down"), ("mixed", "Mixed")]:
        cmp = compare_pvalues(
            r[col_r].values, py[col_py].values, max_log10_diff=0.5
        )
...
def test_rank_sum_test_with_correlation_matches_r():
    ...
    out = rank_sum_test_with_correlation(
        np.arange(20), stat_vec, correlation=0.05, df=np.inf
    )
    np.testing.assert_allclose(out["less"], r["less"][0], rtol=1e-8, atol=1e-12)
    np.testing.assert_allclose(
        out["greater"], r["greater"][0], rtol=1e-8, atol=1e-12
    )
...
def test_gene_set_test_ranks_only_matches_r():
    ...
    for _, row in r.iterrows():
        ...
        cmp = compare_pvalues(
            np.array([row["p"]]), np.array([py]), max_log10_diff=0.5
        )
...
def test_convest_matches_r():
    ...
    np.testing.assert_allclose(
        convest(p), r["pi0_default"][0], rtol=1e-6, atol=1e-12
    )
    np.testing.assert_allclose(
        convest(p, niter=200), r["pi0_200iter"][0], rtol=1e-6, atol=1e-12
    )
...
def test_prop_true_null_all_methods_match_r():
    p = _load("R_propTrueNull_input.csv")["p"].values
    r = pd.read_csv(
        FIXTURES / "R_propTrueNull.csv", header=None, names=["method", "pi0"]
    )
    for _, row in r.iterrows():
        py = prop_true_null(p, method=row["method"])
        np.testing.assert_allclose(
            py, row["pi0"], rtol=1e-6, atol=1e-12,
